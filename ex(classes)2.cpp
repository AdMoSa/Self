//Program to illustrate the use of objects as arguments to member, and non-member functions using Call by value mechanism.
...
void Time::convert(Time t, char ch)   //Member function
{
    switch(ch)
    {
...
int main()
{
    ...
    char ch;
    cin>>ch;
    tm1.convert(tm2,ch);   //object passed on to member function
    tm1.sum(tm1,tm2);
    prnvalues(tm2);        //object passed on to non-member function
    return 0;
}
